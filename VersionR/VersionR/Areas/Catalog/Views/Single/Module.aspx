...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <h3>
        Aktuelle Version
        <%: Model.GetLatestVersion().GetVersionString() %>
    </h3>
    <h5>
        Erschienen am: <small>
            <%: Model.GetLatestVersion().ReleaseDate.ToString("d", CultureInfo.CreateSpecificCulture("de-DE"))%></small>
        Platform: <small>
            <%: Model.GetLatestVersion().Platform%></small>
    </h5>
    <h5>
        Beschreibung / Changelog:</h5>
    <p>
        <%: Model.GetLatestVersion().Description%>
    </p>
    <div class="btn-group">
        <%: Html.ActionLink("Zurück zum Modulkatalog", "Index", new { }, new { @class = "btn" })%>
        <% if (Model.Customer_Modules.All(cm => cm.User.EMail != User.Identity.Name))
           {%>
        ...
        <% }
           ...
           { %>
        <a href="<%= Url.Action("Download", "Version", new {id = Model.GetLatestVersion().VrId, area="Account"}) %>"
